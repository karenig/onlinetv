...
class qqFileUploader {
    ...
    function handleUpload($uploadDirectory, $replaceOldFile = FALSE){
        ...
        if ($this->file->save($uploadDirectory . $filename . '.' . $ext)){
            return array('success'=>true,'image'=>$filename.'.'.$ext);
        } else {
...
$result = $uploader->handleUpload($root_dir);

$name = $result['image'];

include(ADMIN_DIR.'/includes/model/SimpleImage.php');
$image = new SimpleImage();
$image->load($root_dir.$name);
$image->resizeToWidth(600);
$image->save($root_dir.'news-'.$name);

unlink($root_dir.$name);
$url = $root_href.'/'.$id.'/'.'news-'.$name;
